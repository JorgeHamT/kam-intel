...
test("generates controlled why_flagged narratives and a cautious recommendation when confidence is low", () => {
  const result = aggregateRestaurantAssessment(
    agentFixtures.restaurants[3],
    defaultAgentConfig,
    {
  ...
  assert.ok(
    result.assessment.whyFlagged.some((line) =>
      line.includes("Nivel de confianza:"),
    ),
  );
  assert.ok(
    result.assessment.whyFlagged.some((line) =>
      line.includes("Nota de validación:"),
    ),
  );
  assert.equal(
    result.assessment.recommendedAction.code,
    "validate_data_before_action",
  );
  assert.equal(
    result.assessment.nextBestStep.code,
