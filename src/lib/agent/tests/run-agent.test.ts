...
test("runAgent orchestrates detection, scoring, recommendations, aggregation and alert generation", () => {
  ...
  assert.ok(result.validationOverlays.some((overlay) => overlay.degradedByValidation));
  ...
  assert.equal(typeof result.summary.verticalRiskSummary[0]?.vertical, "string");
  assert.equal(typeof result.summary.interventionSummary[0]?.recommendationCode, "string");
});
